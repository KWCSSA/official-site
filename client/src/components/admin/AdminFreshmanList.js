...
class AdminFreshmanList extends React.Component {
  state = { posts: [], updated: false, editing: null, editingState: null, adding: false, addingState: null, modal: false, modalImg: null }
      ...
      return (
        <div className="row">
          ...
          <div className="col-1 d-flex align-items-center">
            <a className="d-flex" href={post.link} target="_blank" rel="noopener noreferrer"><h4 style={{ textDecoration: "underline" }}>链接</h4></a>
          </div>
          <div className="col-1">
            <button className="btn btn-outline-dark d-flex justify-content-center align-items-center" style={{ width: "100%", height: "50px" }} onClick={() => { this.handleEditClick(post.id) }}><i className="material-icons">edit</i></button>
            <button className="btn btn-outline-danger d-flex justify-content-center align-items-center" style={{ width: "100%", height: "50px", marginTop: '10px' }} onClick={() => { this.handleDeleteClick(post.id) }}><i className="material-icons">delete_forever</i></button>
          </div>
        </div>
      );
    };

    const SortableEventItem = SortableElement(({ value }) => <li className="list-group-item">{renderListItem(value)}</li>);
        ...
        <li className="list-group-item">
          <div className="row">
            <div className="col-11 d-flex flex-column">
              <h4>标题：</h4>
              <input value={post.title} style={{ width: "100%", fontSize: "1.5rem", marginBottom: "20px" }} onChange={e => this.handleInputChange('title', e.target.value)} />
              <h4>链接：</h4>
              <input value={post.link} style={{ width: "100%" }} onChange={e => this.handleInputChange('link', e.target.value)} />
            </div>
            <div className="col-1">
