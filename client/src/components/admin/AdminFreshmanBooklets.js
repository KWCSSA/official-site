...
class AdminFreshmanMessage extends React.Component {
	...
	handleConfirmClick() {
		this.props.updateFreshmanBooklets(this.state.editingState, this.props.adminPassword);
		this.setState({
			editing: null,
			editingState: null
		});
					...
					<div className='row'>
						<div className='col-11'>
							<div className='list-group-item d-flex flex-column'>
								...
								<img
									className='mb-3'
									src={this.state.editingState.newStudentBooklet.pic}
									style={{ width: '50%', cursor: 'zoom-in' }}
									alt=''
									onClick={() => this.toggleModal(this.state.editingState.newStudentBooklet.pic)}
								/>
								<h5>封面：</h5>
								<input
									className='mb-3'
									type='file'
									accept='image/jpeg'
									onChange={e => this.handlePicFileSelect('NSBooklet', e.target.files[0])}
								/>
								<h5>PDF文件：</h5>
								<input
									className='mb-3'
									...
									onChange={e => this.handlePdfFileSelect('NSBooklet', e.target.files[0])}
								/>
							</div>
							<div className='list-group-item d-flex flex-column'>
								<h4>安全手册：</h4>
								<img
									className='mb-3'
									src={this.state.editingState.safetyBooklet.pic}
									style={{ width: '50%', cursor: 'zoom-in' }}
									alt=''
									onClick={() => this.toggleModal(this.state.editingState.safetyBooklet.pic)}
								/>
								<h5>封面：</h5>
								<input
									className='mb-3'
									type='file'
									accept='image/jpeg'
									onChange={e => this.handlePicFileSelect('SFBooklet', e.target.files[0])}
								/>
								<h5>PDF文件：</h5>
								<input
									className='mb-3'
									type='file'
									accept='application/pdf'
									onChange={e => this.handlePdfFileSelect('SFBooklet', e.target.files[0])}
								/>
							</div>
						</div>
						<div className='col-1'>
					...
					<li className='list-group-item'>
						<div className='row'>
							<div className='col-5'>
								<h4>新生手册：</h4>
								<img
									src={this.state.newStudentBooklet.pic}
									style={{ width: '80%', cursor: 'zoom-in' }}
									alt=''
									onClick={() => this.toggleModal(this.state.newStudentBooklet.pic)}
								/>
								<a
									className='d-flex'
									href={this.state.newStudentBooklet.link}
									target='_blank'
							...
							<div className='col-5'>
								<h4>安全手册：</h4>
								<img
									src={this.state.safetyBooklet.pic}
									style={{ width: '80%', cursor: 'zoom-in' }}
									alt=''
									onClick={() => this.toggleModal(this.state.safetyBooklet.pic)}
								/>
								<a className='d-flex' href={this.state.safetyBooklet.link} target='_blank' rel='noopener noreferrer'>
									<h4 style={{ textDecoration: 'underline' }}>链接</h4>
								</a>
							</div>
							<div className='col-1'>
