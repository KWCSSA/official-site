...
class AdminEventList extends React.Component {
	...
	renderEvents() {
		...
		const SortableEventItem = SortableElement(({ value }) => (
			<li className='list-group-item'>{renderEventItem(value)}</li>
		));
						...
						<div className='col-10 d-flex flex-column'>
							<div className='d-flex flex-column'>
								...
								<div className='mt-3 mb-3' style={{ height: '500px' }}>
									<img
										...
										src={event.pic}
										alt={event.title}
										onClick={() => this.toggleModal(event.pic)}
									/>
								</div>
							...
							<div>
								...
								<input
									...
									onChange={e => this.handleInputChange('highlight', e.target.checked)}
