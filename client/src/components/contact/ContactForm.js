		...
		<React.Fragment>
			<div className='input-group mb-2'>
				<textarea
					{...input}
					type={type}
					className='msg-content form-control'
					placeholder={placeholder}
					aria-label={placeholder}
					required={required}
				/>
		...
		return (
			<React.Fragment>
				<form className='w-100' onSubmit={this.props.handleSubmit(this.props.onSubmit)}>
					<div
						className='mb-3 d-flex align-items-center send-email'
					...
					<Field type='text' name='name' component={ContactFormInput} placeholder='姓名 (Name) *' required={true} />
					<Field type='email' name='email' component={ContactFormInput} placeholder='邮箱 (Email) *' required={true} />
					<Field
						type='tel'
						name='phone'
						...
						required={false}
					/>
					<Field type='text' name='subject' component={ContactFormInput} placeholder='主题 (Subject) *' required={true} />
					<Field
						type='text'
						name='message'
					...
					<div className='row'>
						<div className='col-8'>
							<div
								className={
									this.props.msgResponse && this.props.msgResponse.success ? (
